...
力学假定：条间剪力 X_i = 0，严格满足每个土条的竖向和水平力平衡（合力平衡法）。
通过引入滑弧切深比 (d/L) 的 Janbu 经验公式修正系数 f0 弥补未考虑力矩平衡的误差：Fs = f0 * F0。
"""
import numpy as np
...
class JanbuSolver(BaseLEMSolver):
    def solve(self) -> Tuple[Optional[float], str]:
        driving = sum(s.W * np.tan(s.alpha) for s in self.slices)
        if driving <= 0:
        ...
        for it in range(self.max_iter):
            ...
            for s in self.slices:
                ...
                if abs(n_alpha) < 1e-5:
                    n_alpha = 1e-5
                num += (s.c * s.b + (s.W - s.u * s.b) * np.tan(s.phi)) / n_alpha
        ...
        # 针对 c-phi 土的经验曲线拟合系数 (Janbu 1973)
        b1 = 0.69
